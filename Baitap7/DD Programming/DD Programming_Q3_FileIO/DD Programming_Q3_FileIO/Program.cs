...
class Program
{
    static void Main()
    {
        ...
        try
        {
            // Tạo file nếu chưa tồn tại + thêm header
            if (!File.Exists(filePath))
            {
                using (StreamWriter sw = File.CreateText(filePath))
                {
            ...
            while (true)
            {
                Console.Write("Nhập tên (Exit để thoát): ");
            ...
            Console.WriteLine($"Số bản ghi đã thêm: {recordCount}");
        }
        catch (IOException ex)
        {
            Console.WriteLine("Lỗi file: " + ex.Message);
        }
    }
}*/
...
class Program
{
    static void Main()
    {
        ...
        try
        {
            ...
            using (StreamWriter sw = new StreamWriter(outputFile))
            {
                ...
                for (int i = 1; i < lines.Length; i++)
                {
                    ...
                    DateTime dateBorrowed = DateTime.ParseExact(parts[2].Trim(), "d/M/yyyy", CultureInfo.InvariantCulture);
                    DateTime dateReturned = DateTime.ParseExact(parts[3].Trim(), "d/M/yyyy", CultureInfo.InvariantCulture);
                    ...
                    // In ra console
                    Console.WriteLine($"{bookID,-10} {borrowerID,-12} {dateBorrowed:yyyy-MM-dd} {dateReturned:yyyy-MM-dd} {daysLoaned,-12} {daysOverdue,-13} ${fine:F2}");

                    // Ghi file nếu quá hạn
                    if (daysOverdue > 0)
                    {
                        sw.WriteLine($"{bookID},{borrowerID},{daysOverdue},${fine:F2}");
                    }
        ...
        {
            Console.WriteLine("Không tìm thấy file loaninfo.csv!");
        }
        catch (FormatException)
        {
            Console.WriteLine("Lỗi định dạng dữ liệu ngày!");
        }
        catch (IOException ex)
        {
            Console.WriteLine("Lỗi file: " + ex.Message);
